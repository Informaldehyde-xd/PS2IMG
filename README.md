...
The UI is a two-pane file browser: a dark sidebar of actions on the left,
and a breadcrumb-navigable folder view on the right (folders as a 2-column
grid, files as a list below — tap a folder tile to drill into it, tap any
breadcrumb segment to jump back up).

- **New Image** — start an empty virtual drive.
- **Open Image** — parses an existing exFAT+MBR image back into a browsable
  ...
  app wrote.
- **Add** (bottom bar) — pick one or more files (e.g. your `.iso`/`.bin` game
  dumps) from your device/cloud storage and add them into the current folder.
...
- **Delete Selected** — check items (via the small checkbox on each tile/row)
  and remove them.
- **Search box** — live-filters the current folder's contents by name.
- **Save** (bottom bar) — writes a fresh exFAT+MBR image to wherever you
  choose (local storage, SD card, etc.), ready to serve from Neutrino's
  UDPBD backing store. Shows live progress for large images.
- **Settings** — set the volume label shown when the drive is mounted, and
  see basic image stats.
...
PS2IMG/
  app/src/main/java/com/ps2imgmanager/
    MainActivity.kt          UI wiring: sidebar, breadcrumb, search, bottom bar, dialogs
    EntryAdapter.kt           Grid+list RecyclerView adapter for the current folder's contents
    image/
  ...
  app/src/main/res/layout/
    activity_main.xml          Sidebar + search/breadcrumb/grid + bottom bar
    item_nav.xml                One sidebar row (icon + label)
    item_folder_grid.xml        One folder grid tile
    item_file_row.xml           One file list row
```
...
- **128 KiB fixed cluster size** — reasonable for GB-scale game files, but
  ...
  formatter would be.
- **No 2nd FAT** — optional per spec, kept out to keep the writer simpler; a
  stricter checker might flag its absence.
